...
Each step is prepended with a number. The first step, for example, is number `00`. Kubernetes manifests associated with the step have the file extension `.yml`. Scripts associated with the step have the file extension e`.sh`. You should run the scripts to apply the manifests, but you can look at them in a text editor or by running `cat` or `less` on them if you like.

The manifests can be read in plain english, though in some cases the scripts are designed to provide a specific environment for them. For this reason, you should not use the manifests _without_ the scripts. In general, the scripts will echo to the terminal what it is that they're doing so that you can follow along.

### NOTE

I tested these scripts on my own workstation for the purposes of making a demo. I run Fedora 33 on my system, and already use `libvirt` extensively - this is the default backend for CodeReady Containers. You may have a small amount of additional setup to go.

As I've got 32 vCPU and 64 GB of RAM, these demo scripts may not work on your system out of the box. There may be resource constraints or simply timing problems that prevent you from succeeding. You should disable components of the control plane (especially Prometheus and Grafana) if you're resource constrained. If you have a hard time getting through these exercises on a lower-power machine due to timeouts, extend the timeouts of the steps - or double them all by default by changing `common.sh`'s `wait_on` function.
...
This step instantiates a basic OpenShift Service Mesh control plane, then waits for it to come up. This step must discreetly occur after operator installation, but doesn't actually deploy any workload to your cluster. You can monitor the control plane installation in your cluster
...
This step applies a simple sidecar configuration that only applies egress policies for the specific mesh members in the `bookinfo` project that prevents them from reaching other mesh members in the cluster. After this simple configuration, another check is applied to validate connectivity to the same URLs as before.

The only URL that should no longer return a status code 200 is the bookinfo-prod service, which is an internal URL resolvable only inside the cluster. The 503 returned here indicates that it is now unavailable. This demonstrates how to isolate namespaces from each other simply using OpenShift Service Mesh.
...
This step locks down the application by first defining explicity a host that it should be able to reach from outside the mesh, then configuring the outboundTrafficPolicy on the `Sidecar` resource to only those services explicitly defined. After the changes have been successfully rolled out, it conducts the same kind of checks performed before, validating connectivity between a number of URLs. The only URL that should be accesible at this point (and therefore return an HTTP 200) is the explicitly defined `git.jharmison.com` host. You may define policy as tightly or widely scoped as desired, including the use of wildcards in some configurations.
